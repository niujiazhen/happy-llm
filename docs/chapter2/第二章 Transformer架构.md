...
#### 1. Attention 机制解决了什么问题？

##### 1.1 RNN / LSTM 的核心问题

- **无法并行计算**（序列依赖，GPU 利用率低）
- **长距离依赖建模能力弱**（信息被压缩进单一隐状态）

##### 1.2 Attention 的核心思想

> 不按顺序“记住”信息，而是**直接建模 token 与 token 之间的关系**

------

#### 2. Attention 的一句话直觉

> **用 Query 决定“关注谁”，再用注意力权重去加权 Value 中的信息**

📌 Attention 不创造新信息，只做 **加权聚合**
...
| 角色  | 含义 | 工程 / 实习视角            |
| ----- | ---- | -------------------------- |
| Query | 查询 | 当前 token 想获取什么信息  |
| Key   | 键   | token 的索引特征，用于匹配 |
| Value | 值   | token 真正携带的内容       |
...
```
scores = Q @ K.T / sqrt(d_k)   # 相似度
attn   = softmax(scores)      # 注意力分布
output = attn @ V             # 信息聚合
```
...
> 因为 QKᵀ 的数值规模会随着维度 dₖ 增大而增大，如果不缩放，softmax 容易进入饱和区，导致注意力分布过于尖锐、梯度变小、训练不稳定。除以 √dₖ 可以控制数值尺度，使训练更加稳定。
...
- 每个 token 同时关注：
  - 自己
  - 其他所有 token
- 生成 **上下文化表示（Contextual Embedding）**

📌 这是 Transformer、BERT、GPT 的核心能力来源

------

#### 7. 如何阅读 Attention Matrix

##### 核心规则（必记）

> **Attention Matrix 的每一行，表示一个 token 在“看谁”**
...
#### 8. Masked Self-Attention（掩码自注意力）

##### 8.1 为什么需要 Mask？

- GPT / Decoder 是 **自回归生成模型**
...
#### 9. Encoder 和 Decoder 为什么不同？

| 模块                   | 任务     | 是否能看未来 | 是否需要 Mask |
| ---------------------- | -------- | ------------ | ------------- |
| Encoder Self-Attention | 理解输入 | ✅ 能         | ❌ 不需要      |
| Decoder / GPT          | 生成输出 | ❌ 不能       | ✅ 需要        |
...
> 一个注意力头只能学习一种关系，多头注意力可以从多个子空间同时建模不同关系。
...
📌 各个 head 的注意力计算是 **完全独立的**

------

#### 13. 为什么 concat 后还要 Linear？
...
- 能解释 √dₖ、softmax、mask 的作用
- 能读懂 MultiHeadAttention 源码
- 能从 Attention Matrix 判断 token 关系

📌 **已达到大模型算法实习对 Attention 的要求**
