...
import ModelConfig
# 这个函数的作用是重复键值对张量，以适应多头注意力机制中的键值对头数量和重复次数
...
# 旋转嵌入RoPE的作用是将输入张量进行旋转，以实现位置编码的效果

# 注意：此处的dim应为 dim//n_head，因为我们是对每个head进行旋转嵌入
def precompute_freqs_cis(dim: int, end: int, theta: float = 10000.0):
    # torch.arange(0, dim, 2)[: (dim // 2)].float()生成了一个从0开始，步长为2的序列，长度为dim的一半
    # 计算频率序列：然后每个元素除以dim，再取theta的倒数，得到频率
    freqs = 1.0 / (theta ** (torch.arange(0, dim, 2)[: (dim // 2)].float() / dim))

    # ：生成时间序列：生成一个从0到end的序列，长度为end
    t = torch.arange(end, device=freqs.device)

    # 计算频率的外积：计算外积，得到一个二维矩阵，每一行是t的元素乘以freqs的元素
...
class Attention(nn.Module):
    def __init__(self, args: ModelConfig):
        super().__init__()
        # 根据是否指定n_kv_heads，确定用于键（key）和值（value）的头的数量。否则就是MHA的头数。
    ...
    def forward(self, x: torch.Tensor, freqs_cos: torch.Tensor, freqs_sin: torch.Tensor):
        """ x
            ...
            Linear 输出
        """
        # 1️⃣ GQA = n_kv_heads < n_heads
        # 2️⃣ RoPE 只作用在 Q 和 K
        # 3️⃣ Flash Attention 自动做 causal mask
        # 4️⃣ 多头计算通过 transpose 实现
        ...
        # 调整形状以适应头的维度。
        # Reshape为多头[batch_size, seq_len, n_local_heads, head_dim]，其中n_local_heads是每个并行处理单元的头数，head_dim是每个头的维度。
        ...
        # 将头作为批次维度处理。因为Attention的计算是针对每个头独立进行的，所以我们将头维度和批次维度合并，以便在后续计算中能够同时处理所有头。
        xq = xq.transpose(1, 2)
        xk = xk.transpose(1, 2)
        xv = xv.transpose(1, 2)
...
if __name__ == "__main__":
    ...
    # Test precompute_freqs_cis and apply_rotary_emb functions
