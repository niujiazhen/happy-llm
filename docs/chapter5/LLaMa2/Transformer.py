...
class Transformer(PreTrainedModel):
    ...
    def __init__(self, args: ModelConfig = None):
        ...
        #Decoder层
        self.layers=torch.nn.ModuleList()# ModuleList便于在forward函数中迭代调用每一层
        for layer_id in range(args.n_layers):
            self.layers.append(DecoderLayer(layer_id, args))

        # RMSNorm层
        self.norm = RMSNorm(args.dim, eps=args.norm_eps)
        # 输出层，线性变换将模型维度映射到词汇表大小
        self.output = nn.Linear(args.dim, args.vocab_size, bias=False)

        self.tok_embeddings.weight = self.output.weight  # 将词嵌入层的权重与输出层的权重共享

        # 预计算频率张量，用于GQA中的旋转位置编码
        freqs_cos, freqs_sin = GQA.precompute_freqs_cis(args.dim // args.n_heads, args.max_seq_len)
        self.register_buffer("freqs_cos", freqs_cos, persistent=False)  # 注册为模型的缓冲区
        self.register_buffer("freqs_sin", freqs_sin, persistent=False)  # 注册为模型的缓冲区
    ...
    def forward(self, tokens: torch.Tensor, targets: Optional[torch.Tensor] = None, **kwargs) -> torch.Tensor:
        ...
        if targets is not None:
            ...
            # 计算交叉熵损失，并将其存储在self.last_loss属性中
        ...
        # 将logits和last_loss存储在self.OUT中，以便在调用时返回
