...
- **预训练+微调范式**。同样在 2018年，ELMo 的诞生标志着预训练+微调范式的诞生。ELMo 模型基于双向 LSTM 架构，在训练数据上基于语言模型进行预训练，再针对下游任务进行微调，表现出了更加优越的性能，将 NLP 领域导向预训练+微调的研究思路。而 BERT 也采用了该范式，并通过将模型架构调整为 Transformer，引入更适合文本理解、能**捕捉深层双向语义关系的预训练任务 MLM**，将预训练-微调范式推向了高潮。
...
BERT 是针对于 NLU 任务打造的预训练模型，其输入一般是文本序列，而输出一般是 Label，例如情感分类的积极、消极 Label。但是，正如 Transformer 是一个 Seq2Seq 模型，使用 Encoder 堆叠而成的 BERT 本质上也是一个 Seq2Seq 模型，只是没有加入对特定任务的 Decoder，因此，为适配各种 NLU 任务，在模型的最顶层加入了一个分类头 **prediction_heads**，用于**将多维度的隐藏状态通过线性层转换到分类维度**（例如，如果一共有两个类别，prediction_heads 输出的就是两维向量）。
...
如图3.5所示，已经通过 Embedding 层映射的 hidden_states 进入核心的 attention 机制，然后通过残差连接的机制和原输入相加，再经过一层 **Intermediate 层**得到最终输出。Intermediate 层是 BERT 的特殊称呼，其实就是一个线性层加上激活函数，用于**想一想通过Attention层看到的东西**：
...
注意，BERT 所使用的激活函数是 **GELU 函数**，全名为**高斯误差线性单元激活函数**，这也是自 BERT 才开始被普遍关注的激活函数。GELU 的计算方式为：

$$GELU(x) = 0.5x(1 + tanh(\sqrt{\frac{2}{\pi}})(x + 0.044715x^3))$$

GELU 的核心思路为**将随机正则的思想引入激活函数，通过输入自身的概率分布，来决定抛弃还是保留自身的神经元**。关于 GELU 的原理与核心思路，此处不再赘述，有兴趣的读者可以自行学习。

BERT 的 注意力机制和 Transformer 中 Encoder 的 自注意力机制几乎完全一致，但是 BERT 将**相对位置编码融合在了注意力机制中**，将相对位置编码同样视为可训练的权重参数，如图3.6所示：
...
+ BERT 是由多层 Transformer Encoder 堆叠而成的 Encoder-Only 预训练模型，通过在顶层加入任务相关的 prediction head 来适配不同 NLU 任务。
+ Encoder 负责“理解语言”，prediction head 负责“适配任务”。
+ BERT 的语义建模能力是在 Encoder 的多层 Self-Attention 中完成的，
  prediction head 只是对已经学好的高维语义表示做任务映射，本身不承担语义理解功能。

#### （3）预训练任务——MLM + NSP

相较于基本沿承 Transformer 的模型架构，BERT 更大的创新点在于其提出的**两个新的预训练任务上——MLM 和 NSP（Next Sentence Prediction，下一句预测）**。预训练-微调范式的核心优势在于，通过将预训练和微调分离，完成一次预训练的模型可以仅通过微调应用在几乎所有下游任务上，只要微调的成本较低，即使预训练成本是之前的数倍甚至数十倍，模型仍然有更大的应用价值。因此，可以进一步扩大模型参数和预训练数据量，使用海量的预训练语料来让模型拟合潜在语义与底层知识，从而让模型通过长时间、大规模的预训练获得强大的语言理解和生成能力。
...
但是，LM 预训练任务的一大缺陷在于，其**直接拟合从左到右的语义关系，但忽略了双向的语义关系**。虽然 Transformer 中通过位置编码表征了文本序列中的位置信息，但这和直接拟合双向语义关系还是有本质区别。例如，BiLSTM（双向 LSTM 模型）在语义表征上就往往优于 LSTM 模型，就是因为 BiLSTM 通过双向的 LSTM 拟合了双向语义关系。因此，有没有一种预训练任务，能够既利用海量无监督语料，又能够训练模型拟合双向语义关系的能力？

基于这一思想，Jacob 等学者提出了 **MLM (Masked Language Model)**，也就是**掩码语言模型**作为新的预训练任务。相较于模拟人类写作的 LM，MLM 模拟的是“完形填空”。MLM 的思路也很简单，**在一个文本序列中随机遮蔽部分 token，然后将所有未被遮蔽的 token 输入模型，要求模型根据输入预测被遮蔽的 token**。例如，输入和输出可以是：

    输入：I <MASK> you because you are <MASK>
    输出：<MASK> - love; <MASK> - wonderful

由于模型可以**利用被遮蔽的 token 的上文和下文一起理解语义来预测被遮蔽的 token**，因此通过这样的任务，模型可以**拟合双向语义**，也就能够更好地实现文本的理解。同样，MLM 任务**无需对文本进行任何人为的标注，只需要对文本进行随机遮蔽即可**，因此也可以利用互联网所有文本语料实现预训练。例如，BERT 的预训练就使用了足足 3300M 单词的语料。
...
在具体进行 MLM 训练时，会随机选择训练语料中 15% 的 token 用于遮蔽。但是这 15% 的 token 并非全部被遮蔽为 `<MASK>`，而是**有 80% 的概率被遮蔽，10% 的概率被替换为任意一个 token，还有 10% 的概率保持不变**。其中 10% 保持不变就是为了消除预训练和微调的不一致，而 10% 的随机替换核心意义在于迫使模型保持对上下文信息的学习。因为如果全部遮蔽的话，模型仅需要处理被遮蔽的位置，从而仅学习要预测的 token 而丢失了对上下文的学习。通过引入部分随机 token，模型无法确定需要预测的 token，从而被迫保持每一个 token 的上下文表征分布，从而具备了对句子的特征表示能力。且由于随机 token 的概率很低，其并不会影响模型实质的语言理解能力。

**本质：MLM 让 BERT 在无监督条件下显式学习深层双向语义表示，这是其在 NLU 任务上超越传统 LM 的根本原因。**



除去 MLM，BERT 还提出了另外一个预训练任务——NSP，即下一个句子预测。NSP 的核心思想是**针对句级的 NLU 任务，例如问答匹配、自然语言推理等**。问答匹配是指，输入一个问题和若干个回答，要求模型找出问题的真正回答；自然语言推理是指，输入一个前提和一个推理，判断推理是否是符合前提的。这样的任务都需要模型在句级去拟合关系，**判断两个句子之间的关系**，而不仅是 MLM 在 token 级拟合的语义关系。因此，BERT 提出了 NSP 任务来训练模型在句级的语义关系拟合。
...
通过**要求模型判断句对关系，从而迫使模型拟合句子之间的关系，来适配句级的 NLU 任务**。同样，由于 NSP 的正样本可以从无监督语料中随机抽取任意连续的句子，而负样本可以对句子打乱后随机抽取（只需要保证不要抽取到原本就连续的句子就行），因此也可以具有几乎无限量的训练数据。
...
可以看到，相比于传统的非预训练模型，其**训练的数据量有指数级增长**。当然，更海量的训练数据需要更大成本的算力，BERT 的 Base 版本和 Large 版本分别使用了 16块 TPU 和 64块 TPU 训练了 4天才完成。

**面试题：**

**1、BERT 和传统语言模型（LM）的核心区别是什么？**

传统 LM 是单向建模上下文，而 BERT 通过 MLM 在预训练阶段显式学习双向语义表示，因此在理解类任务上表现更好。

**2、为什么 MLM 比传统 LM 更适合 NLU？**

MLM 通过同时利用上下文预测被遮蔽词，使模型学到全局双向语义表示，更符合理解类任务的需求。

**3、为什么 BERT 的 MLM 要用 80/10/10，而不是全 Mask？**

80/10/10 的设计用于缓解预训练与下游任务输入分布不一致的问题，并防止模型只关注被 Mask 的位置而忽略上下文建模。

**4、NSP 在 BERT 中的作用是什么？**

NSP 用于让模型学习句子之间的语义与连贯性关系，补充 MLM 只能建模 token 级语义的不足。

#### （4）下游任务微调

作为 NLP 领域里程碑式的成果，BERT 的一个重大意义就是正式确立了预训练-微调的两阶段思想，即**在海量无监督语料上进行预训练来获得通用的文本理解与生成能力，再在对应的下游任务上进行微调**。该种思想的一个重点在于，预训练得到的强大能力能否通过低成本的微调快速迁移到对应的下游任务上。

针对这一点，BERT 设计了更通用的输入和输出层来适配多任务下的迁移学习。对每一个输入的文本序列，BERT 会在其首部加入一个**特殊 token** `<CLS>`。在后续编码中，该 token 代表的即是**整句的状态**，也就是**句级的语义表征**。在进行 NSP 预训练时，就使用了该 token 对应的特征向量来作为最后分类器的输入。`[CLS]` 在**预训练阶段被设计为承载句级监督信号，因此其向量逐渐演化为全局语义表示。**

在完成预训练后，针对每一个下游任务，只需要使用一定量的全监督人工标注数据，对预训练的 BERT 在该任务上进行微调即可。所谓微调，其实和训练时更新模型参数的策略一致，只不过在特定的任务、更少的训练数据、更小的 batch_size 上进行训练，更新参数的幅度更小。对于绝大部分下游任务，都可以直接使用 BERT 的输出。例如，对于文本分类任务，可以**直接修改模型结构中的 prediction_heads 最后的分类头**即可。对于序列标注等任务，可以集成 BERT 多层的隐含层向量再输出最后的标注结果。对于文本生成任务，也同样可以取 Encoder 的输出直接解码得到最终生成结果。因此，BERT 可以非常高效地应用于多种 NLP 任务。
...
**总结：BERT 通过预训练学习通用语义表示，再通过 `[CLS]` 和轻量 prediction head 的微调机制，高效迁移到各类 NLU 下游任务。**
...
实验结果证明，后两组显著优于前两组，且**单文档的 MLM 组在下游任务上微调时性能最佳**。因此，RoBERTa 在预训练中去掉了 NSP，只使用 MLM 任务。

同时，RoBERTa 对 MLM 任务本身也做出了改进。在 BERT 中，Mask 的操作是在数据处理的阶段完成的，因此后期预训练时同一个 sample 待预测的 `<MASK>` 总是一致的。由于 BERT 共训练了 40 个 Epoch，为使模型的训练数据更加广泛，BERT 将数据进行了四次随机 Mask，也就是每 10个 Epoch 模型训练的数据是完全一致的。而 RoBERTa 将 Mask 操作放到了训练阶段，也就是动态遮蔽策略，从而让**每一个 Epoch 的训练数据 Mask 的位置都不一致**。在实验中，动态遮蔽仅有很微弱的优势优于静态遮蔽，但由于动态遮蔽更高效、易于实现，后续 MLM 任务基本都使用了动态遮蔽。

**总结：RoBERTa 证明了：BERT 的性能瓶颈不在架构，而在预训练任务和训练策略。**

+ 实验表明 NSP 任务过于简单，模型可以通过表层语义或主题信息完成判断，对下游任务帮助有限，甚至可能引入噪声，因此 RoBERTa 选择去掉 NSP。
+ 通过在更长的连续文本上进行 MLM 训练，模型可以隐式学习句子之间的语义和连贯关系，而不依赖显式的 NSP 监督。
...
在 BERT 的基础上，RoBERTa 进一步探究了更大规模预训练的作用。同样是基于 BERT 架构进行优化的 ALBERT 模型，则从是否能够减小模型参数保持模型能力的角度展开了探究。通过对模型结构进行优化并对 NSP 预训练任务进行改进，ALBERT 成功地以**更小规模的参数实现了超越 BERT 的能力**。虽然 ALBERT 所提出的一些改进思想并没有在后续研究中被广泛采用，但其降低模型参数的方法及提出的新预训练任务 SOP 仍然对 NLP 领域提供了重要的参考意义。
...
**总结：将词向量维度与隐藏层维度解耦，通过低维 Embedding + 线性映射，大幅减少参数量而不损失表达能力。**
...
**总结：所有 Encoder 层共享同一组参数，显著压缩模型规模，但计算次数不变，训练与推理速度提升有限。**

#### （3）优化三：提出 SOP 预训练任务

类似于 RoBERTa，ALBERT 也同样认为 NSP 任务过于简单，在预训练中无法对模型效果的提升带来显著影响。但是不同于 RoBERTa 选择直接去掉 NSP，ALBERT 选择改进 NSP，增加其难度，来优化模型的预训练。

在传统的 NSP 任务中，正例是由两个连续句子组成的句对，而负例则是从任意两篇文档中抽取出的句对，模型可以较容易地判断正负例，并不能很好地学习深度语义。**而 SOP 任务提出的改进是**，**正例同样由两个连续句子组成，但负例是将这两个的顺序反过来。**也就是说，模型不仅要**拟合两个句子之间的关系，更要学习其顺序关系**，这样就大大提升了预训练的难度。例如，相较于我们在上文中提出的 NSP 任务的示例，SOP 任务的示例形如：
...
**总结：通过句子顺序预测替代 NSP，构造更难的负样本，迫使模型学习真正的句间语义与逻辑关系。**
...
如图3.8所示，从整体来看 T5 的模型结构包括 Tokenizer 部分和 Transformer 部分。Tokenizer 部分主要负责将输入文本转换为模型可接受的输入格式，包括分词、编码等操作。Transformer 部分又分为 **EncoderLayers 和 DecoderLayers** 两部分，他们分别由一个个小的 Block组成，每个 Block 包含了**多头注意力机制、前馈神经网络和 Norm 层**。Block 的设计可以使模型更加灵活，像乐高一样可以根据任务的复杂程度和数据集的大小来调整 Block 的数量和层数。
...
和 Encoder 不一样的是，在 Decoder 中还包含了 **Encoder-Decoder Attention** 结构，用于捕捉**输入和输出序列之间的依赖关系**。这两种 Attention 结构几乎完全一致，只有在位置编码和 Mask 机制上有所不同。如图3.9所示，Encoder 和 Decoder 的结构如下：
...
这种归一化有助于**通过确保权重的规模不会变得过大或过小来稳定学习过程**，这在具有许多层的深度学习模型中特别有用。
...
- 输入格式: 预训练时，T5将输入文本转换为**"文本到文本"的格式**。对于一个给定的**文本序列**，随机**选择一些token进行遮蔽**，并用特殊的占位符(token)替换，**相当于替换掉一段文本为masked**。然后将被遮蔽的token序列作为模型的输出目标。
...
对于不同的NLP任务，每次输入前都会加上一个**任务描述前缀**，明确指定当前任务的类型。这不仅帮助模型在预训练阶段学习到不同任务之间的通用特征，也便于在微调阶段迅速适应具体任务。例如，任务前缀可以是“summarize: ”用于摘要任务，或“translate English to German: ”用于翻译任务。
...
GPT，即 **Generative Pre-Training Language Model**，是由 OpenAI 团队于 2018年发布的预训练语言模型。虽然学界普遍认可 BERT 作为预训练语言模型时代的代表，但首先明确提出预训练-微调思想的模型其实是 GPT。GPT 提出了通用预训练的概念，也就是在海量无监G督语料上预训练，进而在每个特定任务上进行微调，从而实现这些任务的巨大收益。虽然在发布之初，由于性能略输于不久后发布的 BERT，没能取得轰动性成果，也没能让 GPT 所使用的 Decoder-Only 架构成为学界研究的主流，但 OpenAI 团队坚定地选择了不断扩大预训练数据、增加模型参数，在 GPT 架构上不断优化，最终在 2020年发布的 GPT-3 成就了 LLM 时代的基础，并以 GPT-3 为基座模型的 ChatGPT 成功打开新时代的大门，成为 LLM 时代的最强竞争者也是目前的最大赢家。
...
在前面我们说过，BERT 之所以可以采用预训练+微调的范式取得重大突破，正是因为其选择的 MLM、NSP 可以在海量无监督语料上直接训练——而很明显，CLM 是更直接的预训练任务，其**天生和人类书写自然语言文本的习惯相契合，也和下游任务直接匹配**，相对于 MLM 任务更加直接，可以在任何自然语言文本上直接应用。因此，CLM 也可以使用**海量的自然语言语料进行大规模的预训练**。

**总结：GPT 通过在 self-attention 中加入 causal mask，将当前位置之后的 attention score 设为 −∞，从而在 softmax 后使其权重为 0，实现并行训练下的自回归建模。**
...
在decoder层中，hidden_states会经历一系列的处理，这些处理由多个decoder block组成。每个decoder block都是模型的核心组成部分，它们**负责对hidden_states进行深入的分析和转换**。在每个decoder block内部，首先是一个**masked self-attention层**。在这个层中，模型会分别计算query、key和value这三个向量。这些向量是通过hidden_states线性变换得到的，它们是计算注意力权重的基础。然后使用**softmax**函数计算attention score，这个分数反映了不同位置之间的关联强度。通过attention score，模型能够确定在生成当前词时，应该给予不同位置的hidden_states多大的关注。然后，模型将**value向量与attention score相乘，得到加权后的value**，这就是attention的结果。

在完成masked self-attention层之后，hidden_states会进入**MLP层**。在这个多层感知机层中，模型通过两个全连接层对hidden_states进行进一步的特征提取。第一个全连接层将hidden_states映射到一个中间维度，然后通过激活函数进行非线性变换，增加模型的非线性能力。第二个全连接层则将特征再次映射回原始的hidden_states维度。
...
LLaMA模型以其**技术创新、多参数版本、大规模预训练和高效架构设计**而著称。模型支持从7亿到数百亿不等的参数量，适应不同规模的应用需求。LLaMA-1以其开源性和优异性能迅速受到社区欢迎，而LLaMA-2和LLaMA-3进一步通过引入**分组查询注意力机制**和支持**更长文本输入**，显著提升了模型性能和应用范围。特别是LLaMA-3，通过采用128K词表大小的高效tokenizer和15T token的庞大训练数据，实现了在多语言和多任务处理上的重大进步。Meta对模型安全性和社区支持的持续关注，预示着LLaMA将继续作为AI技术发展的重要推动力，促进全球范围内的技术应用和创新。

**总结：通过高质量数据、大量训练 token 以及架构优化（RMSNorm、SwiGLU、RoPE），在较小参数规模下实现接近 GPT-3 的性能。**

+ RMSNorm：只做归一化（无均值中心化）
+ SwiGLU：更强表达能力，实证效果优于 GELU
+ RoPE（旋转位置编码）：更好泛化到长序列，更自然地编码相对位置信息，推理时可扩展上下文长度
...
GLM 最初是由清华计算机系推出的一种通用语言模型基座，其核心思路是在**传统 CLM** 预训练任务基础上，**加入 MLM** 思想，从而构建一个**在 NLG 和 NLU 任务上都具有良好表现的统一模型**。

在整体模型结构上，GLM 和 GPT 大致类似，均是 Decoder-Only 的结构，仅有三点细微差异：

1. **使用 Post Norm 而非 Pre Norm**。Post Norm 是指在进行残差连接计算时，先完成残差计算，再进行 LayerNorm 计算；而类似于 GPT、LLaMA 等模型都使用了 Pre Norm，也就是先进行 LayerNorm 计算，再进行残差的计算。相对而言，Post Norm 由于在残差之后做归一化，对**参数正则化**的效果更强，进而模型的鲁棒性也会更好；Pre Norm相对于因为有一部分参数直接加在了后面，不需要对这部分参数进行正则化，正好可以**防止模型的梯度爆炸或者梯度消失**。因此，**对于更大体量的模型来说，一般认为 Pre Norm 效果会更好**。但 GLM 论文提出，使用 **Post Norm 可以避免 LLM 的数值错误**（虽然主流 LLM 仍然使用了 Pre Norm）；
...
GLM 的核心创新点主要在于其提出的 **GLM（General Language Model，通用语言模型）**任务，这也是 GLM 的名字由来。GLM 是一种结合了**自编码思想**和**自回归思想**的预训练方法。所谓自编码思想，其实也就是 MLM 的任务学习思路，在输入文本中随机删除连续的 tokens，要求模型学习被删除的 tokens；所谓自回归思想，其实就是传统的 CLM 任务学习思路，也就是要求模型按顺序重建连续 tokens。
...
在 23年 6月，智谱就开源了 ChatGLM2-6B。相对于一代，ChatGLM2 将上下文长度扩展到了 32K，通过更大的预训练规模实现了模型性能的大幅度突破。不过，在 ChatGLM2 中，模型架构就基本回归了 LLaMA 架构，引入 MQA （Multi-Query Attention)的注意力机制，预训练任务也回归经典的 CLM，放弃了 GLM 的失败尝试。
...
- **BERT (Google, 2018)**：基于Transformer编码器的堆叠（Encoder-Only架构）。BERT-base 有12层编码器（768维，约1.1亿参数），BERT-large 有24层（1024维，约3.4亿参数）。输入经 tokenizer 转为 token 序列，经嵌入层和位置编码后送入多层自注意力网络。模型顶层加 **prediction head**（分类头），将[CLS]向量映射为下游任务类别。
  - **预训练任务**：MLM（Masked Language Model）+NSP（Next Sentence Prediction）。MLM随机遮蔽约15%词汇，利用左右双向上下文预测被遮蔽词，从而学习深层双向语义；NSP判断两句话是否连续，强化句间关系学习。
  - **关键技术**：采用WordPiece子词分词；位置编码使用可训练的相对位置嵌入（线性层实现，相较Transformer的Sinusoidal绝对编码可捕获更丰富的相对位置信息）；激活函数为GELU；层归一化（LayerNorm）与残差连接使模型训练稳定；参数量大，嵌入层输出维度与隐藏层同维（后续ALBERT优化）。
  - **优化对比**：BERT引入双向预训练范式，确立了预训练+微调模式。后续模型如RoBERTa、ALBERT等在此基础上优化：如RoBERTa去除NSP、扩大数据集；ALBERT参数共享和SOP任务等。BERT自身也有MacBERT、DistilBERT、ERNIE等变体不断迭代。
  - **适用场景**：侧重NLU任务（文本分类、自然语言推理、问答等理解类任务）。下游微调时，通常在顶层添加轻量分类/回归头，例如文本分类取[CLS]向量加线性层，序列标注结合所有隐层输出等。预训练得到通用语义表示后，仅需少量任务数据微调即可。
  - **小结**：BERT首创大规模双向Transformer预训练，在GLUE、MultiNLI等基准上达SOTA，标志着预训练模型时代的开启。其MLM任务解决了传统左到右语言模型对上下文学习不足的问题。局限在于预训练与微调任务存在不一致（下游无MASK），以及最大序列长度受限。
- **RoBERTa (Facebook, 2019)**：在BERT-大型架构基础上改进得到的模型，模型结构与BERT-large相同（24层编码器，1024维，约3.4亿参数）。主要从训练策略和数据集规模上优化。
  - **预训练任务**：仅使用MLM，不再做NSP。研究表明NSP过于简单，对下游帮助有限，甚至引入噪声，因此RoBERTa去掉NSP，只在更长连续文档上做Mask。
  - **训练策略**：采用更大规模语料（160GB，比BERT的13GB大约10倍，包括BookCorpus、维基百科、CC-NEWS、OpenWebText、Stories等）；使用更大batch（8192 vs BERT的256）和更长训练步（总共500K步，全部512长度训练）；mask策略改为动态遮蔽：在每个训练步随机遮蔽，从而每轮见到不同的遮蔽样本；词表使用BPE编码，大小增加到50K（BERT用30K）。
  - **效果与意义**：经改进后，RoBERTa在多项下游任务上刷新了BERT系列SOTA。证明了BERT的瓶颈在训练策略和数据量而非模型结构：更大的数据、更长序列、动态遮蔽等显著提升性能。同时确认仅靠MLM（无NSP）即可学习句间关系：使用单文档连续MLM时微调效果最好。
  - **适用场景**：与BERT相同，面向NLU任务，下游微调方式类似BERT。
  - **小结**：RoBERTa在原BERT架构上简化预训练任务和扩充训练资源，通过训练优化而非结构改动取得更高性能，成为后续模型设计的参考；也说明增大训练规模对模型能力提升至关重要。
- **ALBERT (Google, 2019)**：A Lite BERT，通过架构和训练任务优化大幅压缩参数而不损失性能。仍为Encoder-Only结构，但对BERT做了两大改动，并替换NSP任务。
  - **架构优化**：①**嵌入分解**：将词嵌入输出维度（E）与隐藏层维度（H）解耦，例如设置E=128而H可达1024/2048；在词嵌入后加线性映射将维度升至H。这样Embedding层参数从V×H变为V×E+E×H，大幅减少参数量。②**跨层参数共享**：所有Transformer层共享同一组参数；实际通过一个层结构反复执行多次而不是各层各自参数。因而24层结构也仅有一层参数，大幅压缩模型体积（ALBERT-xlarge 59M参数对比BERT-large的334M）。
  - **预训练任务**：MLM + SOP（Sentence Order Prediction）。SOP用连续句子对的正序/逆序作为正负样本，要求模型判断句子顺序，从而学习句间逻辑。相较NSP仅判别“是否在一起”，SOP更难，需要捕捉句子前后文顺序关系。实验发现MLM+SOP优于MLM+NSP且优于仅MLM。
  - **差异对比**：与BERT相比，ALBERT节约参数的方法显著，但每步计算量与BERT相似（共享参数不减少计算层数）。BERT用Post-LN（隐藏层后LayerNorm）结构，ALBERT沿用原结构；激活函数也是GELU。ALBERT通过架构创新验证小参数模型也可获得超越BERT的性能。
  - **适用场景**：同样针对NLU任务，训练和微调流程与BERT类似。微调时在任务顶部添加head即可；由于参数极少，也便于在资源受限场景部署。
  - **小结**：ALBERT展示了参数因式分解与层参数共享的有效性，实现更小模型的高效学习；SOP任务增强了句序关系建模。但共享参数设计使训练速度并无大幅提升，成为其局限之一。ALBERT的思路为后续轻量化模型设计提供参考。

## 3.2 Encoder-Decoder PLM

- **T5 (Google, 2020)**：Text-To-Text Transfer Transformer，采用Encoder-Decoder结构。输入输出均为文本序列，所有任务统一为文本到文本的转换（text-to-text），大大简化了任务设计。
  - **架构**：Transformer编码器与解码器对称堆叠，输入由编码器处理，解码器生成输出，两者通过注意力层交互。与原始Transformer不同，T5使用相对位置编码以及RMSNorm替代LayerNorm。RMSNorm（均方根归一化）仅有可学习缩放参数，更简单且更稳健。每层仍包含多头自注意力、前馈网络和归一化层。
  - **预训练任务**：主要是**span-corruption MLM**（掩码语言模型），与BERT类似但操作统一为“文本到文本”形式。随机选择输入文本中15%的token遮蔽，解码器任务为恢复这些被遮蔽的文本片段（使用特殊占位符表示遮蔽位置）。预训练用的是大规模C4语料（Colossal Clean Crawled Corpus，约750GB）。T5也尝试多任务混合训练，使模型学到更加通用的表示。
  - **统一思想**：T5的**大一统思想**在于用统一格式处理多种NLP任务。例如在输入前加任务前缀：“summarize: …”或“translate English to German: …”，模型即可按提示生成相应输出。这种统一设计提高了多任务学习的灵活性和迁移性，下游微调时只需更换前缀即可应用于不同任务，无需特殊模型结构。
  - **适用场景**：兼顾NLU和NLG，几乎所有文本生成与理解任务都可表示为文本转换。下游微调直接使用同样的text-to-text流程，根据任务提供任务描述前缀，模型自动适配。
  - **小结**：T5通过统一的文本转换框架简化了任务设定和训练流程，使模型更通用，取得了多任务SOTA成绩。相较单一Encoder或Decoder架构，它更灵活但训练成本更高。

## 3.3 Decoder-only PLM

- **GPT (OpenAI)**：Generative Pre-Training，最初版本GPT-1于2018年发布。采用Transformer的解码器部分堆叠。每层仅包含一个带掩码的自注意力子层（Masked Self-Attention）和前馈网络，输入经过Embedding和Sinusoidal绝对位置编码（可训练或预定义）得到hidden states。GPT最初使用Post-LN结构（归一化在残差后），GPT-2改为Pre-LN。
  - **预训练任务**：CLM（Causal Language Model，因果语言模型）。模型只关注序列中每个位置之前的token，通过自回归方式预测下一个词，符合人类书写习惯。例如输入“今天天气”，模型预测下一个词“很好”。CLM可使用海量未标注文本直接训练，天生适配生成任务。
  - **技术特点**：采用**掩码自注意力**：在注意力计算时遮蔽未来位置，使模型只能关注当前位置及之前；激活函数为GELU，前馈层等同于普通Transformer Decoder，但没有Encoder-Decoder交互层；初代GPT输出层经过线性映射到词表预测下一个词。GPT-2/3在架构上扩大层数和宽度，并采用Pre-LN以稳定训练。
  - **GPT系列演进**：GPT-1（12层, 768维, 1.17亿参数, 5GB BooksCorpus）；GPT-2（48层, 1600维, 15亿参数, 40GB自建WebText，预训练采用zero-shot策略）；GPT-3（96层, 12288维, 175B参数, 570GB数据）。规模、数据和能力逐代提升。
  - **适用场景**：主攻NLG和生成式任务（对话、写作、翻译等）。GPT-2/3进一步强调无需微调即可zero-shot/few-shot完成任务；当然也可对特定任务进行微调或少样本提示。下游时通常直接生成结果。
  - **小结**：GPT系列验证了“体量即正义”的思路，大规模自回归预训练带来质的飞跃，GPT-3成为LLM浪潮的基石。缺点是单向语言建模，理解任务需要适当提示或改进；同时模型巨大，计算资源需求高。
- **LLaMA (Meta, 2023)**：Large Language Model from Meta，系列开源基础模型。架构为Decoder-Only，与GPT类似，使用Transformer解码器堆叠。不同之处在于采用了更新的技术：
  - **技术创新**：使用Rotary位置编码（RoPE）来更自然地表示相对位置，可扩展上下文长度；归一化使用RMSNorm；激活函数采用SwiGLU以增强表示能力；LLaMA-2引入Grouped-Query Attention (GQA)来提高多头注意力的效率；LLaMA-3进一步扩充词表至128K和支持8K上下文。
  - **模型规格**：LLaMA-1（2023年2月）：7B/13B/30B/65B参数，超1万亿token数据预训练；LLaMA-2（2023年7月）：7B/13B/34B/70B参数，数据增至2万亿token，上下文长度提升到4096；LLaMA-3（2024年4月）：8B/70B参数，15万亿token，支持8192上下文和128K词表。随着版本迭代，模型规模和训练语料大幅增长。
  - **适用场景**：同样侧重NLG，可用于文本生成、对话、科研探索等。多参数版本适应不同资源需求。开放源码促进研究社区应用。下游微调方式类似GPT系列，可继续训练或提示学习。
  - **小结**：LLaMA以高效架构设计实现了相对小体量下接近GPT-3的性能。其开源和多版本策略推动了AI社区的广泛应用。分组查询注意力、长上下文支持等创新显著提升了大模型的效能和适用性。
- **GLM / ChatGLM (智谱, 2023)**：中文通用语言模型系列，核心思想在于融合自编码（MLM）和自回归（CLM）预训练。架构为Decoder-Only，整体与GPT类似，主要做了三点修改：采用**Post-LN**（先计算残差再归一化）；输出层只用单一线性层（无双层MLP）；激活函数从ReLU改为GELU。
  - **预训练任务**：GLM任务（General Language Model）：对输入文本随机掩蔽连续片段，然后要求模型同时利用上下文预测这些片段，并在片段内部按自回归方式生成。这一混合机制既适配文本生成，也兼顾理解任务。示例：输入“I <MASK> because you <MASK>”，输出"<MASK> - love you; <MASK> - are a wonderful person"。该任务在一定程度上优于同体量BERT系模型。
  - **模型演进**：ChatGLM-6B（6B参数，1万亿token，2K上下文）使用GLM任务首次训练，同时进行了监督微调（SFT）和强化学习（RLHF）；ChatGLM2-6B改用LLaMA架构、经典CLM、扩展上下文到32K，并引入多查询注意力（MQA）；ChatGLM3-6B在数据和步骤上进一步优化，加入函数调用和代码解释功能；GLM-4系统（128K上下文）虽未开源，但其轻量版GLM-4-9B在1万亿多语言语料上训练，已超过LLaMA-3-8B表现。
  - **适用场景**：面向中文指令理解和对话生成，通过Chat格式进行交互。微调方法参考ChatGPT流程，包括SFT+RLHF等使模型更适应实际对话需求。
  - **小结**：GLM系列首次将MLM与CLM结合于同一模型，在中文LLM发展中具有创新意义；首代ChatGLM开辟了国内开源中文大模型先河。不过随着模型规模放大，研究表明传统CLM更为有效，因此后续版本多回归CLM预训练。GLM的尝试为中文预训练任务提供了重要思路，即使未持续沿用，其设计思路仍具参考价值。
